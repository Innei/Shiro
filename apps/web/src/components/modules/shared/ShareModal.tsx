...
export const ShareModal: FC<ShareData> = ({ url, text, title }) => {
  ...
  return (
    <div className="relative grid grid-cols-[200px_auto] gap-5">
      <div className="inline-block size-[200px] bg-zinc-200/80 dark:bg-zinc-800/90">
        <QRCodeSVG
          value={url}
      ...
      <div className="flex flex-col gap-2">
        ...
        <ul className="w-[200px] flex-col gap-2 [&>li]:flex [&>li]:items-center [&>li]:space-x-2">
          {shareList.map(({ name, icon, onClick }) => (
            <li
              key={name}
              className="flex cursor-pointer items-center space-x-2 rounded-md px-3 py-2 text-lg transition-colors hover:bg-zinc-100 dark:hover:bg-zinc-800"
